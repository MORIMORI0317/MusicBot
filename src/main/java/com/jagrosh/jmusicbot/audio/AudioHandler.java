...
import com.jagrosh.jmusicbot.ikisugi.Dwonloader;
...
/*
        if (qtrack.getTrack().getDuration() != 9223372036854775807l && qtrack != null && qtrack.getTrack() != null && qtrack.getTrack().getInfo() != null) {
            boolean flag = true;
            String id = "";
            if (qtrack.getTrack().getInfo().uri.equals(qtrack.getTrack().getInfo().identifier)) {
                if (Dwonloader.instans().existsSoundFile(qtrack.getTrack().getInfo().uri)) {
                    flag = false;
                    id = Dwonloader.instans().canFolders(qtrack.getTrack().getInfo().uri);
                }
            } else {
                if (Dwonloader.instans().existsYoutube(qtrack.getTrack().getInfo().identifier)) {
                    flag = false;
